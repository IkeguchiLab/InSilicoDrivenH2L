    ...
    with open(results_file_path, 'r') as f:
        ...
        for l in f:
            ...
            if 'CGI: dG' in l:
                ...
                BAR_conv=float(foo[-1])
                BAR_conv_abs =abs(BAR_conv)
            elif 'BAR: Conv Std Err (bootstrap) =' in l:
                BAR_conv_bootstrip=(foo[-1])
    result_list=[edge_dir+"_"+run_dir, CGI_dG, CGI_Std_Err_parametric, CGI_Std_Err_bootstrip, BAR_dG, BAR_Std_Err_bootstrip, BAR_Std_Err_analytical, num_forward, num_backward, BAR_conv, BAR_conv_abs, BAR_conv_bootstrip]
    out.append(result_list)
    print(result_list)

df = pd.DataFrame(out, columns=["edge_run", "CGI_dG", "CGI_Std_Err_parametric", "CGI_Std_Err_bootstrip", "BAR_dG", "BAR_Std_Err_bootstrip", "BAR_Std_Err_analytical", "num_forward", "num_backward", "BAR_conv", "BAR_conv_abs", "BAR_conv_bootstrip"])
df.to_csv(f"{proj_home}workpath/summary/results_conv.csv")
