...
all_dirs=$(ls -d edge_lig_*_lig_*/state*/ligand*/)

results=$(find edge_lig_*_lig_*/state*/ligand*/ -name 'merged.itp')

found_dirs=$(echo "$results" | awk -F'/' '{print $1"/"$2"/"$3"/"}' | sort | uniq)

not_found_dirs=$(comm -23 <(echo "$all_dirs" | sort) <(echo "$found_dirs" | sort))

echo "dir contains merged_itp file:"
echo "$found_dirs" | awk -F'/' '{print $1}' | sort | uniq -c

echo "dir does not contain merged_itp file:"
